...
## V11 — 판단 기록 + 회고 루프: "가격 히스토리가 없다"는 전제가 틀렸다

**설계 문서가 하드 리밋으로 적어둔 제약이 실제로는 존재하지 않았다.**
이 루프는 2026-08-26에 설계해두고 미구현 상태였는데, 그때 메모에
"Yahoo 엔드포인트는 현재가만 주고 과거 구간은 못 준다 - 그래서 판단을
나중에 채점할 방법이 없다"가 하드 리밋으로 적혀 있었다. 이번에 실제로
찔러보니 틀렸다. `market_data.py`가 현재가에 이미 쓰고 있던 바로 그
chart 엔드포인트가 `range=2y`를 받으면 `indicators.quote[0].close`에
전체 종가 배열을 준다 - AAPL로 확인했더니 2년치 501개 포인트가 그대로
나왔다. 새 데이터 소스도, 새 키도, 유료 API도 필요 없었고, 기능 전체를
막고 있던 게 검증 안 된 전제 하나였다. `fetch_price_history()` +
`close_on_or_after()` 두 함수가 추가의 전부다.

**`close_on_or_after()`가 "가장 가까운 값"이 아니라 "이후 첫 값"인
이유는 실제 실패 모드다.** 판단일과 지평일 양쪽 다 주말·공휴일에
떨어지는 게 일상이다. 가장 가까운 종가를 고르면 판단일이 월요일일 때
직전 금요일 가격이 잡히는데, 그건 **판단이 내려지기 전 가격으로 판단을
채점하는 것**이다. 항상 앞으로 스냅하면 최악의 경우 며칠 늦은 가격을
쓸 뿐이고, 방향이 뒤집히는 일은 없다.

**루프를 실제로 돌려보니 연구 메모의 발견이 그대로 재현됐다.**
120일 전(2026-05-14) 날짜로 AAPL $212(안전마진 −82.2%), MSFT
$505(−209.1%) 두 건을 로그에 넣고 90일 지평으로 채점했더니:

```
  AAPL 2026-05-14: overvalued -> +1.4% (wrong)
  MSFT 2026-05-14: overvalued -> +20.3% (wrong)
```

둘 다 고평가 판단이었고 둘 다 올랐다. 표본 2개로 결론을 내릴 순 없지만,
이게 정확히 이 루프를 만든 이유다 - 기본 가정이 우량 대형주를 체계적으로
고평가로 부르는 패턴을, 이제 코드가 기록하고 채점한다.

**회고(LLM) 단계는 아직 실데이터로 못 돌렸다.** `.env`의 Anthropic
키가 세션 도중 무효화돼서(같은 키로 어닝콜 분석은 성공했었다) 401이
떴다. 다만 그 실패가 뜻밖의 검증이 됐다: `resolve_decisions.py`가
종목별로 예외를 잡아 로그를 망가뜨리지 않고 계속 진행했고, 회고가
안 써진 판단은 미해결로 남아 다음 실행 때 자동 재시도된다(멱등성).
`write_reflection()` 자체는 fake 클라이언트로 프롬프트 조립·레코드
생성·토큰 기록까지 단위테스트로 덮었다.

**append-only를 고른 이유는 감사 추적보다 실패 모드 쪽이 컸다.**
기록을 제자리에서 고쳐 쓰지 않으니, 쓰다 만 write가 손상시킬 수 있는
건 항상 마지막 줄 하나뿐이다. `read_records()`가 파싱 안 되는 줄을
경고와 함께 건너뛰게 해두면, 한 줄이 깨져도 히스토리 전체가 못 읽는
파일이 되지 않는다. 테스트로 직접 재현해서 확인(`test_a_corrupt_last_line_does_not_make_the_history_unreadable`).

**정성 리스크를 개수로만 저장한 건 프라이버시 약속과의 충돌 때문이다.**
`app/api/analysis.py`에는 "요청의 어떤 것도 request/response 사이클보다
오래 살지 않는다"는 약속이 주석으로 박혀 있었는데, 디스크에 쓰는 기능을
추가하는 순간 그게 거짓이 된다. 그래서 레코드는 심각도별 개수만 담는다 -
리스크 이름도, 인용문도, `earnings_call_text`의 어떤 조각도 안 들어간다.
테스트는 파싱된 모델이 아니라 **디스크에 실제로 쓰인 바이트**에 대고
확인한다. API 쪽 주석도 "무엇은 쓰이고 무엇은 절대 안 쓰이는지"로
정확히 고쳐 적었다.
